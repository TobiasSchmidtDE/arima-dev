...
bfgs_optimizer = function (X, Z, max_iterations, p, d, q, P, D, Q, s, ncond){
  ...
  while(norm_gx > tol & iter < max_iterations & diff > tol){
    ...
    #4. step
    yk = gxnew - gx
    if (yk == 0) break; # prevent from dividing by zero in the next line
...
custom_arima = function (X, max_iterations, p, d, q, P, D, Q, s){
    ...
    Z = constructPredictorMatrix(diffX, p, d, q, P, D, Q, s) 
...
arimaGrad = function (X, weights, p, d, q, P, D, Q, s){
    browser()
    Z = constructPredictorMatrix (X, p, d, q, P, D, Q, s)
...
RUN ARMA
```{r}
arima_models = arimamodels(X, 2, 3, 0, 3, 0, 0, 0, 2)
...
random_A = rbind(c(10, -1, 2, 0), c(-1, 11, -1, 3), c(2,-1,10,-1), c(0, 3, -1, 8))
random_x = matrix(c(1, 2, -1, 1))
test_b = random_A %*% random_x
calc_x = jacobi_solver(random_A, test_b)
...
#testweights = rep.int(1, 10)
residuals = arimaResiduals (X, testweights,  3, 0, 3, 0, 0, 0, 2)
css = arima_predict_css(X, testweights, 3, 0, 3, 0, 0, 0, 2)
...
#testweights = c(0.2, 0.1, 0.54, 0.2, 0.1, 0.54, 0.02, 0.02)
X = c(1, -2, 3, 2, 1, -2, 3, 2)
testweights = rep.int(2, 4)
grad = arimaGrad (X, testweights, 4, 0, 0, 0, 3)
comparisongrad = compareArima(grad$finite, grad$direct)

print (cbind(grad$finite, grad$direct))

print (comparisongrad)
```
